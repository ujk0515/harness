...
2. **디자이너** → wf_*/desc_*가 있으면 → design_* 생성/수정 (화면이 있으니 디자인이 필요)
3. **개발자** → design_*이 있으면 → 코드 구현/수정 (디자인이 있으니 개발이 필요)
4. **QA** → 코드가 변경되면 → TC 추가/수정
5. **테스터** → TC가 있으면 → Playwright 검증 (개발이 있으니 검증이 필요)
6. 결과 전달
...
- 단순 UI 조정 (색상, 간격, 폰트 크기, 정렬 수정) → 기획자/디자이너가 수정 후 완료
- 요소 표시/숨김 조건 변경 (노출 여부만 바뀌는 경우) → 기획자가 기획서 수정, 필요 시 개발자가 코드 수정 후 자체 확인
