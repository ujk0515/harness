...
VOC/수정 요청 중 아래에 해당하면 **개발자가 자체 검증 후 완료 처리**한다. QA/테스터에게 넘기지 않는다.

- 텍스트 변경 (문구 수정, 오타, 라벨 변경)
- 단순 UI 조정 (색상, 간격, 폰트 크기, 정렬 수정)
- 요소 표시/숨김 조건 변경 (노출 여부만 바뀌는 경우)
- CSS만 수정되는 변경

개발자는 수정 후 브라우저에서 직접 확인하고 결과를 반환한다.

아래에 해당하면 **경량 변경이 아니므로** QA/테스터 검증이 필요하다:
- 새로운 기능 추가
- API 연동 변경
- 데이터 흐름/상태 관리 변경
- 여러 화면에 걸친 동작 변경
