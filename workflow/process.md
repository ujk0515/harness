...
- parent는 `request-state.json`, `.dispatch.json`, ticket 파일을 직접 수정해 우회하지 않는다. 반복 실패 시에도 먼저 validator가 계산한 `retry_scope`로 같은 역할을 다시 호출한다.
- `harness-direct` 같은 우회 처리로 agent 결과를 대체하지 않는다. 하드 스톱 3종 또는 명시적 에스컬레이션일 때만 사용자에게 넘긴다.
...
### 사전 검토 단계
필수 항목이 갖춰진 후, 하네스가 사용자의 기획을 읽고 **결과물을 만드는 데 필요한 빈 구멍만 찾는다.**
**에이전트를 호출하지 않는다. 하네스 자체가 판단한다.**

#### 검토 관점 (기획 구멍 탐지 중심)
- **화면/기능 누락**: 사용자가 기대하는 주요 화면/기능이 정의되지 않았는지
- **상태 누락**: 빈 상태, 에러 상태, 로딩 상태, 성공/실패 후 상태가 빠졌는지
- **입력/출력 기준 누락**: 무엇을 입력받고 무엇을 보여주거나 저장해야 하는지 불명확한지
- **예외 흐름 누락**: 취소, 재시도, 중복 입력, 잘못된 입력, 권한 부족 같은 예외가 빠졌는지
- **외부 의존성 누락**: API, 인증, 결제, 지도, 푸시, 업로드 등 외부 리소스 조건이 비어 있는지
...
- 난이도 표는 **기본적으로 축약**한다. 실제 blocker 가 있거나 난이도 차이가 큰 기능만 짧게 보여준다.

#### 피드백 규칙
- **구체적 해결책을 제시하지 않는다** — 기획은 기획자가 한다
- 문제점 지적 + 빠진 항목 질문만 던진다
- 질문은 **기본 5개 이내**, 정말 구멍이 많을 때만 **최대 10개 이내**
- 질문은 중요도 순으로 정렬한다
- 난이도 표 + 질문 합쳐서 핵심만. 길게 쓰지 않는다
- 아래는 금지한다
  - 사용자 원문에 이미 있는 내용을 다시 묻기
  - 구현 방법 자체를 사용자에게 대신 설계하라고 묻기
  - "어떻게 할까요?", "원하시는 방향은?" 같은 두루뭉술 질문
  - 추측성 질문, 중복 질문, 사소한 취향 질문
...
### 다중 보드 수정 기본 규칙
- 한 dispatch 안에서 여러 보드를 크게 수정해야 하면 한 번에 몰지 말고 **board 단위로 dispatch 를 쪼개는 것**을 먼저 검토한다.
...
2. Agent(planner) 호출: `"[Batch{N}][R{M}][planner] revise: {요청 항목 요약}"` + "시작 순서 고정: 1) `request-workboard.md`와 디자이너 리뷰를 먼저 읽어 2) 수정 대상 `screen_id`와 기존 `wf_*` / `desc_*` 영향을 다시 확인하고 3) 작업 시작 전 `penpot.execute_code`로 현재 페이지의 모든 `wf_*` / `desc_*` Board `{ id, name, type }` 목록을 수집해 `workspace/evidence/planner/Batch{N}/R{M}/wf-desc-snapshot-before.json`에 저장하고 4) 필요한 기획서 + `wf_*` + `desc_*`만 in-place로 수정한 뒤 5) 작업 완료 후 동일 방식으로 `wf-desc-snapshot-after.json`을 저장하고 6) `export_shape` + gap check + claim/evidence + 자가점검까지 끝내기 전에는 완료처럼 말하지 마. Penpot 작업은 꼭 필요한 보드만 만지고 `mcp__penpot__execute_code`는 최대 5회, `export_shape`는 최대 2회만 사용해. 프롬프트에 나열된 Read 파일도 전부 읽지 말고 실제로 필요한 것만 골라 읽어. before에 있던 `wf_*` / `desc_*` Board id는 after에도 모두 남아 있어야 한다. blocked 재호출이면 `request-state.json`의 planner `failed_check_ids` / `retry_scope`를 먼저 읽고 실패한 체크 항목만 보완해. 이미 pass한 항목은 처음부터 다시 하지 마. claim 템플릿: `workspace/claims/Batch{N}/R{M}/planner.claim.json`에 저장하고 최소 `{ batch_id, item_id, role, completion_state, unfinished_reason, covered_items, missing_items, wf_boards, desc_boards, reference_flows, expected_user_path, critical_states, avoid_patterns, export_shape_summary }`를 포함해. evidence는 `workspace/evidence/planner/Batch{N}/R{M}/wf-export.json` + `desc-export.json` + `wf-desc-snapshot-before.json` + `wf-desc-snapshot-after.json`를 남기고, 각 export 파일에는 최소 `{ type, screen_id, board_id, board_name, exported_at }`를 포함해. 디자이너 리뷰: {경로}. 작업 보드: workspace/planning/request-workboard.md. 반영하여 기획서 수정 + `wf_*` + `desc_*` 수정해"
...
1. 루프 A-1/A-2를 통과한 각 `item_id`마다 Agent(designer) 호출: `"[Batch{N}][R{M}][designer] apply: design apply {요청 항목 요약}"` + "시작 순서 고정: 1) `request-workboard.md` + 기획서 + 대응 `wf_*` / `desc_*`를 먼저 읽어 2) 작업 시작 전 `penpot.execute_code`로 현재 페이지의 모든 `wf_*` / `desc_*` Board `{ id, name, type }` 목록을 수집해 `workspace/evidence/designer/Batch{N}/R{M}/wf-desc-snapshot-before.json`에 저장하고 3) planner 구조화 가이드와 현재 화면 상태를 확인한 뒤 `design_*` Board를 생성/수정하고 4) 작업 완료 후 동일 방식으로 `wf-desc-snapshot-after.json`을 저장하고 5) `export_shape` 확인 + gap check + claim/evidence + 자가점검까지 끝내기 전에는 완료처럼 말하지 마. Penpot 작업은 꼭 필요한 보드만 만지고 `mcp__penpot__execute_code`는 최대 5회, `export_shape`는 최대 2회만 사용해. 프롬프트에 나열된 Read 파일도 전부 읽지 말고 실제로 필요한 것만 골라 읽어. before에 있던 `wf_*` / `desc_*` Board id는 after에도 모두 남아 있어야 한다. blocked 재호출이면 `request-state.json`의 designer `failed_check_ids` / `retry_scope`를 먼저 읽고 실패한 체크 항목만 보완해. 이미 pass한 항목은 처음부터 다시 하지 마. claim 템플릿: `workspace/claims/Batch{N}/R{M}/designer.claim.json`에 저장하고 최소 `{ batch_id, item_id, role, action, completion_state, unfinished_reason, developer_ready, developer_reason, developer_targets, request_coverage, covered_items, missing_items, design_boards }`를 포함해. evidence는 `workspace/evidence/designer/Batch{N}/R{M}/design-export.json` + `boards.json` + `wf-desc-snapshot-before.json` + `wf-desc-snapshot-after.json`를 남기고, `design-export.json`에는 최소 `{ type, board_id, board_name, exported_at }`, `boards.json`에는 최소 `{ design_boards }`를 포함해. 기존 와이어프레임 Board는 수정하지 마"
...
3. Agent(planner) 호출: `"[Batch{N}][R{M}][planner] revise: {요청 항목 요약}"` + "시작 순서 고정: 1) 각 item의 `developer-review.md` + `qa-review.md`만 먼저 읽고 2) `수긍` / `반박` / `보완` / `보류`를 정한 뒤 3) 작업 시작 전 `penpot.execute_code`로 현재 페이지의 모든 `wf_*` / `desc_*` Board `{ id, name, type }` 목록을 수집해 `workspace/evidence/planner/Batch{N}/R{M}/wf-desc-snapshot-before.json`에 저장하고 4) 기획서 + 필요시 `wf_*` + `desc_*`를 in-place로 수정한 뒤 5) 작업 완료 후 동일 방식으로 `wf-desc-snapshot-after.json`을 저장하고 6) `export_shape` + gap check + claim/evidence + 자가점검까지 끝내기 전에는 완료처럼 말하지 마. before에 있던 `wf_*` / `desc_*` Board id는 after에도 모두 남아 있어야 한다. blocked 재호출이면 `request-state.json`의 planner `failed_check_ids` / `retry_scope`를 먼저 읽고 실패한 체크 항목만 보완해. 이미 pass한 항목은 처음부터 다시 하지 마. claim 템플릿: `workspace/claims/Batch{N}/R{M}/planner.claim.json`에 저장하고 최소 `{ batch_id, item_id, role, completion_state, unfinished_reason, covered_items, missing_items, wf_boards, desc_boards, reference_flows, expected_user_path, critical_states, avoid_patterns, export_shape_summary }`를 포함해. evidence는 `workspace/evidence/planner/Batch{N}/R{M}/wf-export.json` + `desc-export.json` + `wf-desc-snapshot-before.json` + `wf-desc-snapshot-after.json`를 남기고, 각 export 파일에는 최소 `{ type, screen_id, board_id, board_name, exported_at }`를 포함해. 각 item의 `developer-review.md` + `qa-review.md`만 읽고, 수긍/반박/보완 계획을 반영하여 기획서 + 필요시 `wf_*` + `desc_*`를 수정해. 변경 분류와 반영 여부를 명시해"
4. Agent(designer) 호출: `"[Batch{N}][R{M}][designer] apply: review sync {요청 항목 요약}"` + "시작 순서 고정: 1) 각 item의 `developer-review.md` + `qa-review.md`와 planner 반영 결과만 먼저 읽어 2) 작업 시작 전 `penpot.execute_code`로 현재 페이지의 모든 `wf_*` / `desc_*` Board `{ id, name, type }` 목록을 수집해 `workspace/evidence/designer/Batch{N}/R{M}/wf-desc-snapshot-before.json`에 저장하고 3) 변경 영향이 있는 `design_*`만 재동기화한 뒤 4) 작업 완료 후 동일 방식으로 `wf-desc-snapshot-after.json`을 저장하고 5) `export_shape` 확인 + claim/evidence + 자가점검까지 끝내기 전에는 완료처럼 말하지 마. Penpot 작업은 꼭 필요한 보드만 만지고 `mcp__penpot__execute_code`는 최대 5회, `export_shape`는 최대 2회만 사용해. 프롬프트에 나열된 Read 파일도 전부 읽지 말고 실제로 필요한 것만 골라 읽어. before에 있던 `wf_*` / `desc_*` Board id는 after에도 모두 남아 있어야 한다. blocked 재호출이면 `request-state.json`의 designer `failed_check_ids` / `retry_scope`를 먼저 읽고 실패한 체크 항목만 보완해. 이미 pass한 항목은 처음부터 다시 하지 마. claim 템플릿: `workspace/claims/Batch{N}/R{M}/designer.claim.json`에 저장하고 최소 `{ batch_id, item_id, role, action, completion_state, unfinished_reason, developer_ready, developer_reason, developer_targets, request_coverage, covered_items, missing_items, design_boards }`를 포함해. evidence는 `workspace/evidence/designer/Batch{N}/R{M}/design-export.json` + `boards.json` + `wf-desc-snapshot-before.json` + `wf-desc-snapshot-after.json`를 남기고, `design-export.json`에는 최소 `{ type, board_id, board_name, exported_at }`, `boards.json`에는 최소 `{ design_boards }`를 포함해. 변경이 없으면 그 사유를 남기고, 변경이 있으면 `design_*`를 실제로 다시 맞춰"
