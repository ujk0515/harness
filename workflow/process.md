...
## 기본 원칙
- **풀 하네스는 처음부터 끝까지 끊김 없이 자동 진행하는 것이 기본이다.** 중간에 사용자에게 묻는 것은 예외다.
- 구멍이나 모호함을 발견해도, 합리적 기본값으로 확신 있게 메울 수 있으면 **묻지 말고 그 기본값을 즉시 반영하고 계속 진행한다.** 내린 결정은 "가정"으로 기록만 한다.
- 멈추고 묻는 것은 합리적 추정조차 불가능한 진짜 블로커(아래 `## 중간에 멈추는 경우`)일 때뿐이다.
- **백그라운드 실행 금지.** `run_in_background`(백그라운드 shell)를 쓰지 않는다. 서버·빌드·테스트 포함 모든 명령은 **포그라운드로 실행해 채팅창에 그대로 보이게** 한다. 진행 중인 작업이 항상 화면에 노출돼야 한다. 개발 서버가 필요하면 따로 백그라운드로 띄우지 말고, 테스트 러너(예: Playwright `webServer`)가 한 명령 안에서 서버를 띄웠다 내리도록 구성한다.
...
1. 사용자 요청에서 아래를 먼저 확인한다.
   - 플랫폼
   - 기술 스택
2. 둘 중 하나라도 없으면 요청 맥락으로 합리적 기본값(예: 웹 + 대중적 스택)을 정해 가정으로 적고 진행한다. 맥락상 도저히 추정 불가할 때만 그 항목을 묻는다.
...
4. 사전 검토 단계를 수행한다 (아래 `## 사전 검토 단계` 섹션). 발견한 구멍은 확신 가능한 범위에서 기본값으로 메우고 멈추지 않고 자동 진행한다.
...
### 규칙
- 발견한 구멍은 **확신이 서면 묻지 말고 합리적 기본값으로 메우고 진행한다.** 메운 결정은 작업 보드/planner 프롬프트에 "가정"으로 기록만 한다.
- 사용자 원문에 이미 있는 내용은 건드리지 않는다.
- 합리적 추정조차 불가능하거나 `## 중간에 멈추는 경우`에 해당하는 진짜 블로커만 사용자에게 묻는다. 그 외에는 묻지 않는다.
- 구멍이 없거나 전부 기본값으로 메웠으면 "특이사항 없음(또는 가정 N개 반영), 진행합니다" 한 줄 알리고 바로 다음 단계로 자동 진행한다.

### 흐름
1. 구멍을 합리적 기본값으로 메우고, 그 가정을 작업 보드와 planner 프롬프트에 반영한 뒤 **멈추지 않고 자동으로 본격 시작한다.**
2. 진짜 블로커(`## 중간에 멈추는 경우`)에 해당하는 항목만 그것을 묻고, 나머지는 계속 진행한다.
...
- 재검증(developer 수정 후 qa/tester 재호출)은 **해당 item 범위만** 다시 본다. 이전 item이나 전체 서비스를 처음부터 다시 돌리지 않는다.
- 같은 역할 재호출은 batch당 최대 2회. 한도를 넘기면 진행을 멈추고 남은 open 코멘트를 사용자에게 보고한다.
- **필수 누락은 한도와 별개로 보충 기회를 1회 더 준다.** 재호출 한도를 다 썼더라도, 사용자 원문·원본 기획·상위 요구에 있는데 산출물에서 빠진 "해야 할 내용"이 남아 있으면 그대로 다음 단계로 넘기지 않는다.
  1. 게이트로 무엇이 빠졌는지 확인한다.
  2. 그 빠진 항목만 채우는 **보충 전용 루프 1회**를 부여한다(한도 리셋). 이 루프에서는 빠진 것만 채우고 다른 건 건드리지 않는다.
  3. 빠진 항목이 채워지면 누락 0 → 다음 단계로 진행한다.
  4. 보충 루프 후에도 필수 누락이 남으면, 더 반복하지 않고 그 항목을 "미완료(필수)"로 끝까지 들고 가 최종 보고서에 명시한다.
- 이 규칙은 planner·developer·qa·tester·secretary 모든 역할에 동일하게 적용한다 (각자 자기 단계의 필수 누락 기준).
