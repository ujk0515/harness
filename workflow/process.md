...
|-----------|---------|----------|-----------|-------------|
| 화면 구조/기능 변경 | 필수 | 필수 | 필수 | 필수 |
| 스타일/시각만 변경 | 필수 (`desc_*`/기획 반영) | 필수 (`design_*`) | 필수 | 스모크 검증 |
| 코드 로직만 변경 | 선택 (기획 충돌 시만) | 불필요 | 필수 | 필수 |
| 문구/텍스트만 변경 | 필수 (기획서) | 기본 불필요, 단 레이아웃 영향 시 조건부 | 필수 | 스모크 검증 |
...
3. **developer** → 기획 또는 디자인이 변경되면 코드 구현/수정
4. **QA** → 코드가 변경되면 TC 추가/수정 또는 정적 검증
5. **tester** → 코드가 변경되면 Playwright 검증
6. 결과 전달
...
- 단순 UI 조정 (색상, 간격, 폰트 크기, 정렬 수정) → 기획자 수정 + 디자이너가 design_* 반영
