...
**원칙: 화면이 있으면 디자인이 필요하고, 디자인이 있으면 개발이 필요하고, 개발이 있으면 검증이 필요하다.**

각 단계의 산출물이 다음 단계의 입력이 된다. 앞 단계가 산출물을 만들었으면 뒷 단계는 자동으로 이어진다.

1. **기획자** → 기획서 수정 + wf_*/desc_* 생성/수정 (화면이 관련되면)
2. **디자이너** → 아래 중 하나라도 해당하면 design_* 생성/수정:
   ...
   - 디자이너는 default 상태만이 아니라 **해당 화면의 모든 상태(empty, loading, error, disabled)와 인터랙션(모달, 서랍, 토스트, 드롭다운)**도 디자인한다.
3. **개발자** → 기획 또는 디자인이 변경되면 → 코드 구현/수정
4. **QA** → 코드가 변경되면 → TC 추가/수정
5. **테스터** → TC가 있으면 → Playwright 검증
6. 결과 전달

이 흐름은 "규칙이니까 따른다"가 아니라 **각 단계의 산출물이 존재하면 다음 단계가 자연스럽게 이어지는 것**이다. 하네스는 이 흐름을 끊지 않는다.

### 경량 변경 판단

VOC/수정 요청 중 아래에 해당하면 **담당 에이전트가 자체 검증 후 완료 처리**한다. 전체 루프를 돌리지 않는다.

- 텍스트 변경 (문구 수정, 오타, 라벨 변경) → 기획자가 기획서 수정 후 완료
- 단순 UI 조정 (색상, 간격, 폰트 크기, 정렬 수정) → 기획자 수정 + 디자이너가 design_* 반영 후 완료
- 요소 표시/숨김 조건 변경 (노출 여부만 바뀌는 경우) → 기획자 수정 + 화면이 바뀌면 디자이너도 design_* 반영 + 필요 시 개발자가 코드 수정 후 자체 확인
- CSS만 수정되는 변경 → 개발자가 수정 후 자체 확인

담당 에이전트가 수정하고 직접 확인 후 결과를 반환하면 끝. QA/테스터에게 넘기지 않는다.
