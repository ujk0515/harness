...
## 적용 범위
- 이 문서는 메인 클로드가 **하네스 모드로 진입한 뒤** 따르는 규칙이다.
- 진입 트리거(예: "하네스로 해줘", "경량 하네스로 동작해줘") 없이 들어온 요청에는 이 문서를 적용하지 않고 일반 Claude Code 어시스턴트 모드로 동작한다.
- 진입 트리거 규칙은 `CLAUDE.md` 의 `## 0. 하네스 진입 룰` 을 참조한다.
...
- 요소 종류·UIUX 의도 모호 (입력/선택 요소 종류 미정, 색·강조·위계 같은 시각 의도 누락, 전체 화면 스타일 통일 여부)
...
3. 마지막으로 secretary 호출이 끝나면 메인 하네스가 1회 실행:
   - `node .claude/scripts/cycle-init.js --update {통합 문서 경로}`
   - 스크립트가 헤더의 `종료일시`와 `참여 에이전트`를 갱신한다. 헤더를 손으로 고치지 않는다.
...
2. qa `review:` — **기획 게이트 (개발 시작 전)**
   - 계획이 사용자 원문의 요구를 전부 담았는지 1:1 대조한다.
   - 누락/모호가 있으면 `[QA→Planner] (open)` 코멘트 등록 → planner `revise:` 재호출 → qa 재확인.
   - 게이트 통과 전에는 developer로 넘어가지 않는다.
3. developer `review:` (기술 리스크가 크면)
4. developer `implement:`
5. qa `tc:` / `verify:`
6. tester `verify:` (실행 가능한 경우)
   - 이슈 발견 시 `[Tester→Developer] (open)` 등록 → developer 수정 → tester 재확인 (`## 코멘트 루프 닫기` 참조)
7. secretary
...
## 코멘트 루프 닫기 (메인 하네스 책임)
- 각 에이전트 호출이 끝날 때마다 메인 하네스가 통합 문서의 `## [코멘트/이슈]`에서 `(open)` 줄을 확인한다.
- open 코멘트의 받는이가 이미 작업을 마친 역할이면, 다음 역할로 넘어가기 전에 그 역할을 재호출해 처리시킨다.
- qa/tester가 발견한 이슈도 동일하게 처리한다. 기능이 기획과 다르게 동작하면 (심각도와 무관하게) developer에게 `(open)` 코멘트로 보내고, developer를 재호출해 고치게 한 뒤 tester가 재확인한다. "기능 블로커 아님"이라는 이유로 메인 하네스가 임의로 넘어가지 않는다.
- 예외: 사용자가 명시적으로 "이건 넘어가도 됨"이라고 한 항목만 스킵 가능하다.
- 같은 역할 재호출은 batch당 최대 2회. 한도를 넘기면 진행을 멈추고 남은 open 코멘트를 사용자에게 보고한다.
- secretary는 open 코멘트가 0개일 때 호출하는 것이 기본이다. 재호출 한도 초과로 더 처리할 수 없으면 그대로 호출하되, secretary가 미해결 코멘트를 최종 보고서에 숨기지 않고 기록한다.
