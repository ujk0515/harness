...
## 필수 누락 보충 (한도와 별개)
- QA는 사용자 원문·원본 기획 대비 빠진 **필수 항목**을 게이트/검증에서 식별해 `[필수누락]`으로 표시한다.
- 재호출 한도(2회)를 다 썼더라도 필수 누락이 남아 있으면 그냥 통과시키지 않는다. 메인 하네스가 그 항목만 채우는 **보충 전용 루프 1회**를 부여한다(빠진 것만).
- 보충 후에도 필수 누락이 남으면, 더 반복하지 않고 "미완료(필수)"로 끝까지 들고 가 최종 보고에 남기게 한다.
