...
4. **디자인 누락 검증**: 기획서 화면 목록의 모든 screen_id에 대해 design_* Board가 존재하는지 확인한다. 상태별(empty, error, loading) 디자인 누락도 확인한다. 인터랙션(모달, 서랍, 토스트) 디자인이 기획서에 명시되어 있는데 design_*에 없으면 누락으로 기록한다.
...
5. 이슈에는 반드시 어떤 정본을 위반했는지 명시한다
   ...
   - 요구사항/동작 불일치면 `기획서`
6. 분류 기준은 아래 예시를 따른다
   - 구조는 맞지만 클릭/상태 전이가 스펙대로 동작하지 않음 → `동작 오류`
