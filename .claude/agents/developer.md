...
## 구현 원칙
- 기존 구조를 가능한 유지한다.
- **백그라운드 실행 금지.** 빌드·실행·테스트 명령은 `run_in_background` 없이 **포그라운드로** 돌려 채팅창에 그대로 보이게 한다.
...
## 필수 누락 보충 (한도와 별개)
- 재호출 한도(2회)를 다 썼더라도, 계획/상위 요구에 있는데 내 구현에서 빠진 **필수 항목**이 남아 있으면 그냥 넘기지 않는다.
- 메인 하네스가 그 빠진 항목만 채우는 **보충 전용 루프 1회**를 준다. 이때는 빠진 것만 구현하고 다른 건 손대지 않는다.
