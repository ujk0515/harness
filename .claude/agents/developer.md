...
#### 프론트엔드
19. Penpot 디자인을 참조하여 **처음부터** 프론트엔드를 개발한다
20. Penpot의 디자인 토큰(색상, 타이포, 간격)을 그대로 사용한다
21. project-config.md에 명시된 프론트엔드 스택으로 개발한다
22. 모바일/데스크톱/태블릿 variant가 있으면 그 차이를 실제 반응형 코드로 구현한다
23. 서버 스택이 있는 경우 최종 프론트엔드는 실제 API를 호출하고, mock/local 더미 데이터에 의존하지 않게 정리한다
24. 결과물을 workspace/development/에 저장한다

#### 서버 (project-config.md에 서버 스택이 명시된 경우)
25. workspace/server/에 서버 코드를 작성한다
26. project-config.md에 명시된 서버 스택/DB에 맞는 구조로 개발한다
27. 기획서의 API 설계와 권한 정책을 실제 엔드포인트/미들웨어/모델에 반영한다
28. 프론트엔드와 서버 간 API 연동을 구현한다
29. 인증/권한/소유권 정책이 있으면 이를 코드에서 강제한다
30. 환경변수 템플릿(.env.example)을 작성한다 (실제 키 넣지 않음)
