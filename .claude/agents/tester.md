...
2. tester 담당 항목 상태를 `in_progress`로 갱신한다
3. 전달받은 테스트케이스(TC), 개발 결과물 경로, `workspace/planning/project-config.md`를 확인한다
4. 기획서와 필요 화면의 `wf_*`, `desc_*`, `design_*`를 확인하고, 테스트 대상 `screen_id`와 variant를 먼저 정리한다
5. **대상 플랫폼 페이지로 전환한다** — 테스트 대상 variant가 속한 `{프로젝트명} — Mobile/Desktop/Tablet` 페이지를 연다
6. 여러 플랫폼 variant가 있으면 관련 플랫폼 페이지를 순서대로 전환하며 확인한다
7. 프론트엔드 실행 명령을 아래 우선순위로 결정한다
...
8. 서버 스택이 있으면 백엔드 실행 명령을 아래 우선순위로 결정한다
   - `workspace/server/README.md`의 실행 방법
   - `workspace/server/package.json`의 `dev` 또는 `start` 스크립트
   - 위가 없으면 `node index.js`
9. 프로세스를 백그라운드로 실행할 때는 PID를 저장해두고, 종료 시 해당 PID만 정리한다
10. 서버가 실제로 응답 가능한 상태가 될 때까지 대기한 뒤 테스트를 시작한다

#### Step 1-1: Penpot / viewport 준비
11. variant가 있으면 variant별로 뷰포트를 나눈다
...
12. Penpot 프레임 크기를 확인할 수 없으면 일반적인 뷰포트로 보수적으로 대체하되, 결과에 대체 기준을 기록한다
13. `design_*`가 없는 화면은 시각 기대값을 임의로 만들지 않는다
   - 구조/흐름은 `wf_*` + `desc_*` 기준으로 검증하고, `디자인 기준 없음`을 별도로 기록한다

#### Step 2: Playwright 테스트 작성
14. TC를 읽고, Playwright 테스트 코드를 작성한다
15. 테스트 파일을 `workspace/testing/playwright/` 에 저장한다
16. 테스트 코드 작성 원칙:
   - TC 1개 = `test()` 1개로 매핑한다
...
#### Step 3: 테스트 실행
17. Playwright 테스트를 실행한다:
   ```bash
   npx playwright test workspace/testing/playwright/ --reporter=list
   ```
18. 실패한 테스트가 있으면 원인을 분석한다
19. **실패 원인이 테스트 코드 문제인지, 개발 결과물 문제인지 구분한다**
   - 셀렉터 불일치 → index.html의 실제 DOM 구조를 확인하고 테스트 코드를 수정 후 재실행
   - 개발 결과물 문제 → 버그로 기록
20. 반응형 화면이면 주요 variant별로 최소 1회 이상 실제 브라우저 검증을 수행한다
21. 서버 스택이 있으면 프론트 ↔ 서버 연동 흐름까지 확인한다
