...
const reminder = [
  ...
  '  - 목적은 feasibility 일반론이 아니라 **기획 구멍 탐지**다.',
  '  - 화면/기능 누락, 상태 누락, 입력/출력 기준 누락, 예외 흐름 누락, 외부 의존성 누락만 본다.',
  '  - 기능 단위 난이도 표(S/M/H/X)는 blocker 가 있거나 편차가 큰 항목만 짧게 적는다.',
  '  - 구체적 해결책은 제시하지 않는다. 구현 방법을 대신 설계하라고 묻지 않는다.',
  '  - 질문은 중요도 순으로 기본 5개 이내, 정말 구멍이 많을 때만 최대 10개 이내.',
  '  - 사용자 원문에 이미 있는 내용 재질문 금지. "어떻게 할까요?" 같은 두루뭉술 질문 금지.',
  '- 사전 검토 결과 분기:',
  ...
  '2. 사전 검토 단계 (기획 구멍 탐지 중심: 화면/상태/입출력/예외/외부 의존성 → 질문 있음: hold-open, 없음: 자동 진행)',
  '3. 사용자 답변 반영 / hold 해제',
  ...
  '- **Penpot Board 실존 스냅샷 필수:** planner/designer apply 완료 직후 `mcp__penpot__execute_code` 로 현재 파일의 Board 목록을 수집해 `workspace/planning/.penpot-boards.json` 에 `{ "checked_at": "<ISO>", "file_id": "<id>", "boards": [{ "board_id": "<id>", "board_name": "<name>", "page_id": "<page>" }, ...] }` 로 기록한다. validator 가 claim 의 wf_/desc_/design_boards 가 이 스냅샷에 전부 존재하는지 교차검증한다. 30분 초과 stale 이면 fail.',
  ...
  '',
  '### 루프 B (4인 공동 리뷰 + 업무 분배)',
  '- 선행: 루프 A-3 designer `apply:` 완료 후 진입.',
  '- Step 1 — 4인 리뷰 수집 (동시/순차 무관, 4명 다 필수):',
  '  - planner `review:` → `workspace/reviews/{batch}/{item}/planner-review.md`',
  '  - designer `review:` → `workspace/reviews/{batch}/{item}/designer-review.md` (루프 B용, A-uiux-review.md 와 별도)',
  '  - developer `review:` → `workspace/reviews/{batch}/{item}/developer-review.md`',
  '  - qa `review:` → `workspace/reviews/{batch}/{item}/qa-review.md`',
  '  - 모든 리뷰 파일은 `## UIUX 보완점` + `## 디스크립션(desc_*) 보완점` + `## 기획서 보완점` 3개 섹션을 반드시 포함. 본문 120자 이상.',
  '- Step 2 — 메인 하네스가 4개 리뷰를 통합해 업무 분배:',
  '  - `workspace/reviews/{batch}/{item}/assignments.json` 작성',
  '  - 형식: `{ planner_tasks:[{id, source, summary}], designer_tasks:[{id, source, summary}], rationale }`',
  '  - `source` 는 이 task 가 어느 리뷰에서 나왔는지 (planner/designer/developer/qa). `summary` ≥10자, `rationale` ≥20자.',
  '  - 기획 정본(문구/흐름/스코프/상태 정의) 수정은 `planner_tasks`, 시각/레이아웃/design_* 수정은 `designer_tasks` 로 분리.',
  '- Step 3 — 분배 task 처리:',
  '  - planner `revise:` → `assigned_task_ids` 에 assignments.planner_tasks 전원 포함 + wf_/desc_ 수정',
  '  - designer `apply:` (sync) → `assigned_task_ids` 에 assignments.designer_tasks 전원 포함 + design_* 재동기화',
  '- validator 차단:',
  '  - 4인 리뷰 1개라도 누락/섹션 미작성이면 planner `revise:` dispatch 차단',
  '  - assignments.json 누락/형식 오류 차단',
  '  - claim.assigned_task_ids 가 자기 몫을 전부 커버하지 않으면 done 발급 차단',
  '',
  '### 루프 C (구현 + 테스트케이스)',
  ...
  '',
  '### developer 순서 위반 차단 (pre-dispatch 게이트)',
  '- developer `review:` 또는 `implement:` dispatch 직전에 validator 가 아래 순서를 강제한다:',
  '  1. planner done_ticket issued + planner.claim 에 plan 모드 흔적(user_raw_request_quoted/read_log≥4/action_rationale). 없으면 차단.',
  '  2. planner.claim.designer_required 가 `Y` 면 designer done_ticket + designer.claim.mode=`apply` + design_boards nonempty + developer_ready=Y 전부 필수. `review` 모드 claim 만 있으면 차단.',
  '  3. planner.claim.designer_required=`N` 인 경우에만 designer skip_ticket 허용. 추가로 design_reason 30자 이상.',
  '  4. design_* 는 `.penpot-boards.json` 스냅샷에 실존해야 함 (action≠NO_CHANGE).',
  '- 이 게이트는 새 함수 `checkDeveloperHandoffGate` 가 수행. 메시지 형식: `Developer {mode} blocked: ...`. 메인 하네스는 이 메시지를 보고 빠진 단계로 되돌아간다.',
  '',
  '### designer review 점수 게이트 강제',
  '- designer `review:` done 결과 `A-uiux-review.md` 점수가 80점 미만 또는 `review_approval = N` 이면 validator 가 designer `review` done 발급을 차단한다. 이때 메인 하네스는 즉시 planner `revise:` 를 호출해 지적 사항을 기획서/`wf_*`/`desc_*` 에 반영하고 다시 designer `review:` 를 돌려 80점 이상 받을 때까지 반복한다.',
  '- A-2 반복 합산(5회)은 `autoHoldStuckReviewGates` 가 `loop_a2_revise_exhausted` 로 hold-open 한다. 이 hold 가 걸리면 사용자에게 상황 보고.',
  '- 루프 B revise 반복(3회)은 `loop_b_revise_exhausted` 로 hold-open 한다.',
  '',
